...
var submitCoupon = function (code, desc, domain) {
  ...
  chrome.runtime.sendMessage({
    ...
    submitCoupon_callback(response.data);
  });
};
...
var parseCoupons = function (coupons, domain) {
  try {
    ...
    couponDisplay.innerHTML = '<div class="submit-button">Submit Coupon</div>' + '<h1>Coupons</h1>' + '<p>Browse coupons below that have been used for <strong>' + domain + '</strong></p>' + '<p style="font-style:italic;">Click any coupon to copy &amp; use</p>' + '<ul>' + couponHTML + '</ul>';
    couponDisplay.style.display = 'none';
    document.body.appendChild(couponDisplay);
    var couponButton = document.createElement('div');
    couponButton.className = '_coupon__button';
    couponButton.innerHTML = '<img src="https://i.ibb.co/3F8jPkh/canva-logo.png" alt="Bee - Copoun Finder" width="70" height="60"> ';
    ...
    couponSubmitOverlay.innerHTML = '<span class="close">(x) close</span>' + '<h3>Do you have a coupon for this site?</h3>' + '<div><label>Code:</label><input type="text" class="code"/></div>' + '<div><label>Description:</label><input type="text" class="desc"/></div>' + '<div><button class="submit-coupon">Submit Coupon</button></div>';
